...
- **Write every output file to the target folder IMMEDIATELY** after generating it. Do NOT display content in the terminal and consider the job done.
- Do NOT summarize what you "would" write — write the actual file.
- Do NOT ask for confirmation before writing — just write it.
- After writing each file, confirm with: `✅ Written: [full file path]`
- After writing, verify the file exists by running `ls [target path]` and confirm the filename appears in the output.
- If a write fails, report the error immediately and retry once. If it fails again, log it and continue to the next session.
- **A session is NOT complete until the file physically exists on disk and has been verified.**
