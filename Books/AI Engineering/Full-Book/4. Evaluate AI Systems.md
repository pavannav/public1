...
![A pink chart with green and red check marks Description automatically generated](../images/aien_0401.png)
...
![A graph of multiple colored bars Description automatically generated](../images/aien_0402.png)
...
![A diagram of a political system Description automatically generated](../images/aien_0403.png)
...
| Length constraints | Number paragraphs + first word in i-th paragraph | There should be {N} paragraphs. Paragraphs and only paragraphs are separated from each other by two line breaks. The {i}-th paragraph must start with word {first\_word}. |
...
2. Roleplaying as a prompt engineering technique to improve the quality of a model’s outputs, as discussed in [Chapter 5](https://learning.oreilly.com/library/view/ai-engineering/9781098166298/ch05.html#ch05a_prompt_engineering_1730156991195551)
...
![A colorful rectangular bars with text Description automatically generated with medium confidence](../images/aien_0404.png)
...
You are a role−playing performance comparison assistant. You should rank the models based on the role characteristics and text quality of their responses. 
The rankings are then output using Python dictionaries and lists.

User Prompt:

The models below are to play the role of ‘‘{role_name}’’. The role description of ‘‘{role_name}’’ is ‘‘{role_description_and_catchphrases}’’. I need to rank
...
the richer, the better. (If the question contains reference answers, then the role−specific knowledge and memories are based on the reference answer.)
```
...
![A diagram of a diagram Description automatically generated](../images/aien_0405.png)
...
![A diagram of a service Description automatically generated](../images/aien_0406.png)
...
![A graph showing a number of sources Description automatically generated with medium confidence](../images/aien_0407.png)
...
![A screenshot of a graph Description automatically generated](../images/aien_0408.png)
...
| --- | --- | --- |
| Data | - *Have to send your data to model providers, which means your team can accidentally leak confidential info* | - Don’t have to send your data externally - *Fewer checks and balances for data lineage/training data copyright* |
...
| Functionality | - More likely to support scaling, function calling, structured outputs - *Less likely to expose logprobs* | - *No/limited support for function calling and structured outputs* - Can access logprobs and intermediate outputs, which are helpful for classification tasks, evaluation, and interpretability |
...
| Control,  access, and  transparency | - *Rate limits* - *Risk of losing access to the model* - *Lack of transparency in model changes and versioning* | - Easier to inspect changes in open source models - You can freeze a model to maintain its access, *but you’re responsible for building and maintaining model APIs* |
...
2. MMLU ([Hendrycks et al., 2020](https://arxiv.org/abs/2009.03300)): Measuring knowledge and reasoning capabilities in 57 subjects, including elementary mathematics, US history, computer science, and law.
3. HellaSwag ([Zellers et al., 2019](https://arxiv.org/abs/1905.07830)): Measuring the ability to predict the completion of a sentence or a scene in a story or video. The goal is to test common sense and understanding of everyday activities.
4. TruthfulQA ([Lin et al., 2021](https://arxiv.org/abs/2109.07958)): Measuring the ability to generate responses that are not only accurate but also truthful and non-misleading, focusing on a model’s understanding of facts.
5. WinoGrande ([Sakaguchi et al., 2019](https://arxiv.org/abs/1907.10641)): Measuring the ability to solve challenging pronoun resolution problems that are designed to be difficult for language models, requiring sophisticated commonsense reasoning.
...
![A screenshot of a graph Description automatically generated](../images/aien_0409.png)
...
N-gram overlapping :   For example, if a sequence of 13 tokens in an evaluation sample is also in the training data, the model has likely seen this evaluation sample during training. This evaluation sample is considered *dirty*.

Perplexity :   Recall that perplexity measures how difficult it is for a model to predict a given text. If a model’s perplexity on evaluation data is unusually low, meaning the model can easily predict the text, it’s possible that the model has seen this data before during training.
...
![A table of numbers with text Description automatically generated](../images/aien_0410.png)
